...
#fix dates:
numdays = 610
base = datetime.date.fromisoformat('2022-05-01')
...
for ticker in Ticker:
    ...
    financial[['abn_return', 'p_val_rank']].to_excel(path_AR + 'p_val/' + ticker + 'pval.xlsx')
    ...
    fig.savefig('H:/LLongo/Reddit/results/' + ticker + '_AR_test.jpg')
    plt.title(ticker + '  Test period (2022m6-2023m12)')
    plt.show()
    ...
    new_df.mean(axis = 1).to_csv(path_AR + target + "_" + str(h_star) + "_test_" + ticker)
...
for tick in Ticker:
    ti = ti + 1
    if ti == 1:
        df_AR = pd.read_csv(path_AR + target + "_" + str(h_star) + "_test_" + tick, index_col = 0)
    else:
        dff = pd.read_csv(path_AR + target + "_" + str(h_star) + "_test_" + tick, index_col = 0)
...
ax.xaxis.set_major_locator(MaxNLocator(integer=True))
ax.legend(fontsize = 12, frameon = False)
plt.title('I=' + str(I) + ', Nd=' + str(network_days) + ', r=' + str(red) + '  Test period (2022m6-2023m12)') 
fig.savefig('H:/LLongo/Reddit/results/AR_test.jpg')
plt.show()
...
ax.legend(fontsize = 12, frameon = False)
plt.title('I=' + str(I) + ', Nd=' + str(network_days) + ', r=' + str(red) + '  Test period (2022m6-2023m12)') 
fig.savefig('H:/LLongo/Reddit/results/CAR_test.jpg')
plt.show()




file_path = path_AR + 'alert1/test_alert1.xlsx'

with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
    for sheet_name, df in ALERTS.items():
